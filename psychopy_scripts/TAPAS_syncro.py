"""
APPTapping_syncro.py
--------------------
PsychoPy acquisition script for the FREE/DYADIC SYNCHRONIZATION condition.
...
This script runs on the stimulation PC and handles:
  ...
  - Fixed-duration session management (default: 360 seconds)
  - CSV and log file output per subject and block

Unlike APPTapping_metronomo.py, this script does NOT present any auditory metronome.
It is intended for dyadic or free-synchronization paradigms where temporal structure
is generated by the participants rather than an external pacing stimulus.
...
Output files (saved in ./csv_files/ and ./log_files/):
  - Subj_<ID>_<block>.csv : semicolon-separated log with columns: subj; time; event
  - Subj_<ID>_<block>.log : PsychoPy log file
        ...
        if "TAP_EVENT" in line:
            ...
            if clock and tempo_risposta is not None:
                ...
                try:
                    ...
                    pass  # logfile may not yet be initialized if tap occurs before subject ID entry
...
def timer(clock=None):
    """
    Manages the fixed-duration recording session (default: 360 seconds).
    Runs in a dedicated thread and terminates all processes when time expires.

    The session duration can be adjusted by modifying the value in the
    while-loop condition (currently 360 seconds = 6 minutes).

    An 'escape' key check is included but has limited reliability in
    threaded PsychoPy contexts; termination via timer expiry is the
    primary exit mechanism.
    """
    start_time = time.time()
    print("SESSION STARTED")
    while time.time() - start_time < 360:  # Session duration in seconds — adjust as needed
        if event.getKeys(["escape"]):
            break

    # Close all resources after session ends
    print("Session ended. Closing processes.")
    logfile.close()
    if port:
        port.close()
    process_1.terminate()
...
# thread_3 manages the session timer.
...
thread_3 = threading.Thread(target=timer, args=(clock,))

thread_1.start()
thread_2.start()

# ─────────────────────────────────────────────
# SUBJECT ID AND BLOCK DIALOG
# ─────────────────────────────────────────────
# A GUI dialog collects the subject ID and block number before the session starts.
# Block number is appended to the output filename to support multiple blocks per subject.
ID = gui.Dlg(title='Subject ID')
ID.addField('ID: ')
ID.addField('Block: ', choices=['1', '2', '3'])
response = ID.show()
subID = response[0].strip()
filename_suffix = response[1].strip()

filename = logdir + f'/csv_files/Subj_{subID}_{filename_suffix}.csv'
print(f"Output file: {filename}")

if not isinstance(filename_suffix, str):
    print("Error: invalid block identifier.")
    core.quit()

if os.path.exists(filename):
    err = gui.Dlg(title='Attention')
    err.addText(f'FILE Subj_{subID}_{filename_suffix}.csv ALREADY EXISTS!')
    err.show()
    core.quit()
else:
    ...
    filename_log = f"{logdir}/log_files/Subj_{subID}_{filename_suffix}.log"
...
thread_3.start()    # Start session timer thread

# Keep main thread alive until session ends or KeyboardInterrupt
try:
    while True:
        time.sleep(1)
